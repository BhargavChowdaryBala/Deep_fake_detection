...
const TABS = [
  ...
  { id: 'text', label: 'Enter Text', icon: FileText },
];
...
export default function UploadSection({ onResult }) {
  ...
  const getInputData = () => {
    if (activeTab === 'file' && file) return { type: fileType, name: file.name, size: file.size };
    if (activeTab === 'url' && url.trim()) return { type: 'url', name: url.trim() };
    if (activeTab === 'text' && text.trim()) return { type: 'text', name: 'Text Input' };
    return null;
  };
  ...
  return (
    <div className="w-full flex flex-col gap-8 animate-fade-in-up">
      ...
      <div className="w-full glass-elevated rounded-2xl p-6 sm:p-8 flex flex-col gap-6">
        {/* File Tab */}
        {activeTab === 'file' && (
          <div
            onDrop={handleDrop}
            ...
            className={`border border-dashed rounded-xl p-10 sm:p-12 text-center cursor-pointer transition-all duration-300
              ${dragActive
                ? 'border-neon-blue/40 bg-neon-blue/[0.04]'
            ...
            {!file ? (
              <div className="flex flex-col items-center gap-5">
                <div className="w-14 h-14 rounded-xl bg-white/[0.04] border border-white/[0.06] flex items-center justify-center">
                  <Upload className="w-6 h-6 text-neon-blue/60" />
                </div>
                <div className="flex flex-col items-center gap-1">
                  ...
                  <p className="text-white/25 text-sm font-light">
                    Images, Videos, Text files — Max 50MB
                  </p>
                </div>
                <div className="flex gap-5 text-white/15 text-xs">
                  <span className="flex items-center gap-1.5"><Image className="w-3 h-3" /> PNG, JPG, GIF</span>
                  <span className="flex items-center gap-1.5"><Video className="w-3 h-3" /> MP4, WebM</span>
                  <span className="flex items-center gap-1.5"><File className="w-3 h-3" /> TXT, PDF</span>
                </div>
              </div>
            ) : (
              <div className="flex flex-col items-center gap-4" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="flex items-center gap-3">
                  <div className="glass px-4 py-2.5 rounded-lg flex items-center gap-2">
                    ...
                    {fileType === 'text' && <File className="w-4 h-4 text-neon-pink/60" />}
        ...
        {activeTab === 'text' && (
          <div className="flex flex-col gap-3">
            <label className="text-[13px] text-white/40 font-medium">Enter text to analyze</label>
            <textarea
              id="text-input"
              value={text}
              onChange={(e) => setText(e.target.value)}
              placeholder="Paste or type the news article, social media post, or any text you want to verify..."
              rows={6}
              className="w-full p-4 bg-white/[0.025] border border-white/[0.06] rounded-xl text-white/80 text-sm placeholder-white/15 focus:outline-none focus:border-neon-purple/25 transition-all duration-200 resize-none"
            />
            <div className="flex justify-between text-xs text-white/20 font-light">
        ...
        {/* Analyze Button */}
        <AnalyzeButton inputData={getInputData()} onResult={onResult} />
      </div>
    </div>
  );
}
